...
class StateHolderTest extends PropSpec with Matchers {
  ...
  property("Dummy blockchain impl") {
    ...
    val inputs: Seq[Input] = initialOutput.map(output => Input(output.id, Seq(Random.randomBytes())))
    ...
    val state: State = State(initialOutput.map(output => Base16.encode(output.id) -> output).toMap)
